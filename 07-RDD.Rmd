...
A comparação de $\lim_{x \to c^-} E[Y_i | X_i = x]$ com $\lim_{x \to c^+} E[Y_i | X_i = x]$ fornece uma estimativa do salto no resultado observado no cutoff. Escreverei os limites laterais como $x \to c^-$ para aproximação pela esquerda e $x \to c^+$ para aproximação pela direita:
...
extrair_rd_tabela <- function(modelo, nome) {
  data.frame(
    ...
    `IC 95% inferior` = modelo[["ci"]]["Robust", "CI Lower"],
    `IC 95% superior` = modelo[["ci"]]["Robust", "CI Upper"],
    `h esquerda` = modelo[["bws"]]["h", "left"],
    `h direita` = modelo[["bws"]]["h", "right"],
    check.names = FALSE
  )
}
