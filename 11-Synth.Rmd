...
A comparação com regressão, portanto, tem duas faces. Na primeira tentativa acima, a regressão é "horizontal": as unidades de controle são as observações, os preditores pré-tratamento são as variáveis explicativas, e os pesos sobre os estados aparecem indiretamente pela matriz de projeção. No SCM, a lógica vira "vertical": os períodos pré-tratamento passam a ser as observações, os doadores passam a ser as variáveis explicativas, e os pesos são estimados diretamente. As duas leituras conversam porque ambas constroem $\hat{Y}_{tr,t}(0)$ como combinação ponderada dos controles; a diferença é como os pesos são aprendidos e quais restrições são impostas.
...
**Interpretação como regressão vertical.** Essa leitura vertical formaliza a ponte com a regressão. Em vez de estimar, no *donor pool*, uma função que prevê o resultado futuro a partir dos preditores da unidade tratada, o SCM escolhe diretamente pesos sobre os doadores para reproduzir sua trajetória pré-tratamento. Para cada período pré-tratamento $t = 1, \ldots, T_0$, a variável dependente é $Y_{N,t}$, o resultado da unidade tratada, e as variáveis explicativas são $Y_{1,t}, \ldots, Y_{N_0,t}$, os resultados dos doadores. O SCM busca pesos $\omega_i$ que resolvam:

$$
Y_{N,t} \approx \sum_{i=1}^{N_0} \omega_i Y_{i,t}, \quad \text{para } t = 1, \ldots, T_0
$$

Se estimássemos essa regressão vertical por mínimos quadrados sem restrições, voltaríamos a problemas semelhantes aos da regressão horizontal: pesos negativos, extrapolação e baixa auditabilidade. O SCM muda o problema ao impor convexidade. Quando $N_0 > T_0$, há mais doadores do que períodos pré-tratamento. O sistema pode ficar subdeterminado: diferentes combinações ajustam bem o pré-tratamento, mas divergem no pós-tratamento. As restrições de convexidade funcionam como regularização. Elas restringem o espaço de soluções, costumam produzir poucos pesos substantivos e tornam o contrafactual inspecionável.
