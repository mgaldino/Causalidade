...
1. Entender o SCM como uma regressão vertical regularizada e explicar por que as restrições de convexidade funcionam como regularização.
...
**Interpretação como regressão vertical.** Uma forma iluminadora de entender o SCM é vê-lo como uma **regressão "vertical"** — isto é, uma regressão ao longo da dimensão temporal, não da dimensão cross-section. Para cada período pré-tratamento $t = 1, \ldots, T_0$, temos uma "observação" em que a variável dependente é $Y_{N,t}$ (o resultado da unidade tratada) e as variáveis explicativas são $Y_{1,t}, \ldots, Y_{N_0,t}$ (os resultados dos doadores). O SCM busca pesos $\omega_i$ que resolvam:

$$
Y_{N,t} \approx \sum_{i=1}^{N_0} \omega_i Y_{i,t}, \quad \text{para } t = 1, \ldots, T_0
$$

Essa é uma regressão com $T_0$ observações e $N_0$ parâmetros. Quando $N_0 > T_0$ (mais doadores do que períodos pré-tratamento), o sistema é subdeterminado — há infinitas soluções que ajustam perfeitamente os dados pré-tratamento, mas que podem divergir nos períodos pós-tratamento (overfitting). As restrições de convexidade ($\omega_i \geq 0$, $\sum \omega_i = 1$) funcionam como uma forma de **regularização**: restringem o espaço de soluções, produzem tipicamente uma solução **esparsa** (poucos pesos não-nulos) e evitam extrapolação. A esparsidade tem uma vantagem adicional: torna o contrafactual **transparente** — o pesquisador pode inspecionar quais unidades compõem o controle sintético e avaliar se a combinação faz sentido substantivo.

A tabela a seguir ilustra a estrutura do problema. A coluna sombreada é a unidade tratada; as demais são doadores. O SCM é uma regressão das linhas pré-tratamento da coluna sombreada sobre as demais colunas:

|  | Doador 1 | Doador 2 | $\cdots$ | Doador $N_0$ | **Tratada** |
|--|----------|----------|----------|-------------|-------------|
| $t=1$ | $Y_{1,1}$ | $Y_{2,1}$ | $\cdots$ | $Y_{N_0,1}$ | $Y_{N,1}$ |
| $\vdots$ | $\vdots$ | $\vdots$ | | $\vdots$ | $\vdots$ |
| $t=T_0$ | $Y_{1,T_0}$ | $Y_{2,T_0}$ | $\cdots$ | $Y_{N_0,T_0}$ | $Y_{N,T_0}$ |
| $t=T_0+1$ | $Y_{1,T_0+1}$ | $Y_{2,T_0+1}$ | $\cdots$ | $Y_{N_0,T_0+1}$ | $Y_{N,T_0+1}(1)$ |
| $\vdots$ | $\vdots$ | $\vdots$ | | $\vdots$ | $\vdots$ |
| $t=T$ | $Y_{1,T}$ | $Y_{2,T}$ | $\cdots$ | $Y_{N_0,T}$ | $Y_{N,T}(1)$ |

As linhas pré-tratamento (acima da divisória) são usadas para estimar os pesos; as linhas pós-tratamento (abaixo) são usadas para projetar o contrafactual $\hat{Y}_{N,t}(0) = \sum_i \hat{\omega}_i Y_{i,t}$.
