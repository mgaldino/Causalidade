...
**Mapa do capítulo.** O capítulo começa com o modelo 2×2, seus pressupostos, estimação por TWFE e inferência. Em seguida aplica DiD aos dados brasileiros do Obra Transparente e introduz event studies. A parte central trata da "revolução DiD": o viés do TWFE com tratamento escalonado, a contaminação de event studies dinâmicos e os estimadores modernos que corrigem esses problemas. Depois discutimos testes de tendências paralelas, análise de sensibilidade, uma segunda aplicação empírica (enchente do Elbe), covariáveis, extensões importantes e um checklist prático de desenho.
...
### Trajetórias e contrafactual

Para DiD, a figura mais útil não é um DAG de ajuste por confundidores. A identificação vem de uma trajetória contrafactual: usamos a evolução do grupo controle para inferir onde os tratados estariam no pós-tratamento se não tivessem recebido a intervenção.

```{r did-counterfactual-paths, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Tendências paralelas e contrafactual dos tratados. A linha tracejada mostra onde os tratados estariam no pós-tratamento sob a hipótese de tendências paralelas."}
did_paths <- tibble::tribble(
  ~grupo, ~periodo, ~resultado,
  "Controle observado", "Pré", 20,
  "Controle observado", "Pós", 35,
  "Tratado observado", "Pré", 30,
  "Tratado observado", "Pós", 50,
  "Tratado contrafactual", "Pré", 30,
  "Tratado contrafactual", "Pós", 45
)

ggplot2::ggplot(
  did_paths,
  ggplot2::aes(
    x = periodo, y = resultado, group = grupo,
    colour = grupo, linetype = grupo
  )
) +
  ...
  ggplot2::labs(x = NULL, y = "Resultado médio", colour = NULL, linetype = NULL) +
  ggplot2::theme_bw(base_size = 12) +
  ggplot2::theme(legend.position = "bottom")
```

A distância vertical entre o tratado observado e o tratado contrafactual no período pós é o efeito estimado. A diferença de níveis entre grupos é eliminada pela comparação no tempo; a tendência comum é eliminada pela comparação com o controle.
...
Bertrand, Duflo e Mullainathan (2004) mostram o problema com placebos em painéis estaduais: quando há autocorrelação serial e tratamento persistente, erros-padrão robustos convencionais rejeitam a hipótese nula com frequência muito maior do que 5%. A recomendação prática é clusterizar no nível da variação de tratamento ou da atribuição da política. Se a política varia por estado, clusterize por estado; se varia por município, clusterize por município; se os dados estão no nível individual, mas o tratamento é municipal, a unidade relevante para inferência continua sendo o município.
...
Quando há poucos clusters — especialmente poucos clusters tratados — os erros-padrão clusterizados assintóticos podem continuar ruins. Nesse caso, considere wild cluster bootstrap, inferência por randomização quando o mecanismo de atribuição é defensável, ou uma apresentação mais honesta de incerteza baseada no número de clusters. Em event studies, também faz sentido distinguir intervalos ponto-a-ponto de bandas simultâneas: se vários coeficientes de pré-tratamento são inspecionados ao mesmo tempo, bandas simultâneas comunicam melhor a incerteza conjunta (Freyaldenhoven, Hansen e Shapiro, 2019; Goldsmith-Pinkham, 2026).
...
modelsummary(
  list("TWFE" = did_ot_static),
  stars = c("*" = .1, "**" = .05, "***" = .01),
  coef_rename = c("treat_post" = "Tratamento × Pós"),
  gof_map = c("nobs", "r.squared"),
  title = "DiD estático — Obra Transparente (versão JDS)"
)
```
...
wild_results %>%
  ...
  kable(
    ...
    col.names = c("Coeficiente", "Estimativa", "SE cluster", "p cluster", "p wild", "p permutação")
  )
```
...
Goodman-Bacon (2021) mostrou que o estimador TWFE com tratamento escalonado é uma média ponderada de todos os estimadores DiD 2×2 possíveis, com pesos que dependem do tamanho dos grupos e da variância do tratamento. Vamos demonstrar isso com uma simulação.
...
O paper estima DiDs separados por horizonte: um placebo pré-tratamento (1994--1998), um efeito de curto prazo (1998--2002), um efeito de longo prazo (1998--2005) e um efeito de muito longo prazo (1998--2009). Em cada arquivo, `Flooded` é o indicador de tratamento daquele painel de dois períodos: vale 1 para distritos afetados no período pós e 0 caso contrário. Em termos substantivos, Bechtel e Hainmueller estão implementando manualmente a lógica do que hoje chamaríamos de **event study** ou **modelo dinâmico**: estimar um efeito separado para cada horizonte temporal em relação ao evento.
...
msummary(list("Placebo 1994--1998" = did_elbe_placebo,
              "Curto prazo 1998--2002" = did_elbe_2002,
              "Longo prazo 1998--2005" = did_elbe_2005,
              "Muito longo prazo 1998--2009" = did_elbe_2009),
         stars = c('*' = .1, '**' = .05, '***' = .01),
         title = "DiD por horizonte temporal — Enchente do Elbe")
```
...
msummary(list("Estático médio" = did_elbe_estatico,
              "Dinâmico por horizonte" = did_elbe_dinamico),
         stars = c('*' = .1, '**' = .05, '***' = .01),
         title = "Efeito médio versus efeitos por horizonte — Elbe")
```
...
ggplot(efeitos_elbe, aes(x = horizonte, y = estimativa)) +
  ...
  geom_point(size = 2.5) +
  scale_x_continuous(breaks = c(2002, 2005, 2009)) +
  theme_bw() +
...
Borusyak, Jaravel e Spiess (2024) propõem enriquecer o modelo de resultados potenciais não-tratados:
$$
...
- **Covariáveis contínuas**: variáveis que afetam o nível de $Y$ (evitando maus controles — ver abaixo)
- **Tendências lineares específicas por unidade**: $\gamma_i \cdot t$ — cada unidade tem sua própria tendência temporal
...
Uma segunda abordagem, desenvolvida por Abadie (2005) e Sant'Anna e Zhao (2020) para o caso não-escalonado, condiciona as tendências paralelas em características baseline $X_i$:
...
1. **Ponderação por propensity score inverso (IPW):** ponderar as unidades de controle para que a distribuição de $X$ replique a do grupo tratado (Abadie, 2005).
...
Callaway e Sant'Anna (2021) estendem essa lógica para tratamento escalonado, estimando um propensity score separado para cada coorte e período. Wooldridge (2021) propõe uma extensão do TWFE (*extended TWFE*) que inclui interações entre os efeitos fixos de tempo e as covariáveis, obtendo resultados robustos a efeitos heterogêneos sob certas condições.
...
1. **Contaminação por efeitos do tratamento.** Com efeitos heterogêneos, $\hat{\gamma}$ é estimado usando *todas* as observações — inclusive as tratadas. Isso pode misatribuir parte do efeito do tratamento às covariáveis, enviesando tanto $\hat{\gamma}$ quanto $\hat{\tau}$ (Borusyak et al., 2024; Caetano et al., 2024).

2. **Covariáveis variantes no tempo como bad controls.** Quando $X_{it}$ pode ser afetado pelo próprio tratamento (por exemplo, emprego quando o tratamento é um programa social), incluí-lo como controle introduz viés de variável intermediária — o clássico problema de *bad controls* (Angrist & Pischke, 2009). A sabedoria convencional de simplesmente "não incluir" essas variáveis também é insuficiente: quando o PTA condicional depende dessas covariáveis, omiti-las viola a suposição de identificação (Caetano et al., 2024).

3. **Covariate effect bias.** Lin e Zhang (2022) mostram que, mesmo com efeitos do tratamento homogêneos e apenas dois períodos, a inclusão de covariáveis variantes no tempo no TWFE gera um viés adicional — o *covariate effect bias* — que surge porque o efeito das covariáveis sobre $Y$ pode variar no tempo. Esse viés persiste mesmo quando o tratamento não afeta as covariáveis.
...
- **Covariáveis variantes no tempo** ($X_{it}$) exigem cautela. Se o tratamento pode afetar $X_{it}$, incluí-la diretamente é um bad control. Caetano et al. (2024) propõem estratégias que lidam com covariáveis variantes no tempo que podem ser afetadas pelo tratamento — incluindo estimandos doubly-robust e abordagens de imputação que usam o valor que a covariável *teria tido na ausência de tratamento*.

**Regra prática:** sempre que possível, use a abordagem 1 (enriquecer o modelo de $Y(0)$ com estimação apenas nos não-tratados) ou a abordagem 2 (PTA condicional com covariáveis baseline e doubly-robust). Evite adicionar covariáveis variantes no tempo diretamente ao TWFE.
...
onde $s$ indexa estados, $t$ tempo e $g$ grupos. O coeficiente $\tau$ compara a mudança do grupo exposto nos estados tratados com: (i) a mudança do grupo exposto nos estados controles, (ii) a mudança do grupo não exposto nos estados tratados e (iii) a mudança do grupo não exposto nos estados controles. A suposição não é que todas as tendências sejam paralelas; é que a **falha** de tendências paralelas entre estados seria a mesma no grupo exposto e no grupo não exposto.
...
Esse checklist é inspirado nas aulas de Hull (2024) e Goldsmith-Pinkham (2026), que enfatizam que DiD deve começar pela definição do desenho e do estimando, não pela escolha automática de uma regressão.
...
1. **DiD 2×2** é intuitivo e pode ser estimado por TWFE sem problemas.
2. **Tendências paralelas** é o pressuposto de identificação central; é não-testável diretamente, depende da escala do outcome e pode ser avaliado com event studies (com cautela) e análise de sensibilidade (HonestDiD).
...
4. **Com tratamento escalonado e efeitos heterogêneos**, o TWFE pode ser enviesado. A decomposição de Goodman-Bacon revela os pesos problemáticos, e Sun e Abraham mostram a contaminação dos coeficientes dinâmicos.
5. **Estimadores modernos** — Callaway & Sant'Anna, Sun & Abraham, Gardner/did2s, Borusyak et al. — corrigem o problema e convergem para estimativas consistentes.
...
7. **Covariáveis devem ser incluídas com cuidado**: enriquecer o modelo de $Y(0)$ com estimação nos não-tratados ou usar PTA condicional com doubly-robust — nunca adicionando covariáveis variantes no tempo ao TWFE ingenuamente.
...
Caetano, C., Callaway, B., Payne, S., & Rodrigues, H. S. (2024). Difference in Differences with Time-Varying Covariates. Working Paper (arXiv: 2202.02903).
...
Goldsmith-Pinkham, P. (2026). *Applied Empirical Methods*: Difference-in-Differences Lectures, Homework 6, and Comprehensive Exam Practice Questions. Yale University course materials. https://github.com/paulgp/applied-methods-phd

Goldsmith-Pinkham, P., Hull, P., & Kolesár, M. (2024). Contamination Bias in Linear Regressions. *American Economic Review*, 114(12), 4015–4051.

Goodman-Bacon, A. (2021). Difference-in-Differences with Variation in Treatment Timing. *Journal of Econometrics*, 225(2), 254–277.

Hull, P. (2018). Estimating Treatment Effects in Mover Designs. Working Paper (arXiv: 1804.06721).

Hull, P. (2024). *'Metrics Notes*: Parallel Trends, Negative Weights, and Clustering. Brown University lecture slides. https://about.peterhull.net/metrix

Huntington-Klein, N. (2022). *The Effect: An Introduction to Research Design and Causality*. Chapman & Hall/CRC.

Lin, L., & Zhang, Z. (2022). Interpreting the coefficients in dynamic two-way fixed effects regressions with time-varying covariates. *Economics Letters*, 216, 110604.
