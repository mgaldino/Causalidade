...
Durante muito tempo, inferência causal com regressão era caracterizada por recomendações vagas, ad hoc e inconsistências. O debate sobre o efeito causal do cigarro sobre câncer de pulmão é ilustrativo a esse respeito. Durante décadas, pesquisadores enfrentaram dificuldades para diferenciar correlação de causalidade em estudos epidemiológicos sobre tabagismo. Muitos estudos iniciais baseados apenas em correlação eram contestados por não apresentarem mecanismos claros ou critérios objetivos para validar inferências causais. @fisher1958, por exemplo, questionou os resultados iniciais que ligavam o cigarro ao câncer por falta de critérios explícitos para identificar uma relação causal, argumentando que a correlação observada poderia decorrer de fatores confundidores, causalidade reversa ou problemas de mensuração.

A situação mudou gradualmente com contribuições metodológicas importantes, como os critérios de causalidade propostos por @hill1965. Os critérios de Bradford Hill ofereceram uma lista explícita e sistemática para avaliar a plausibilidade causal em estudos observacionais:
...
4. Corresponder a um "experimento natural"
5. Comportar-se apropriadamente quando a causa é aplicada, removida e então reinstalada
6. Ser consistente com o conhecimento especializado do tema
7. Ser, por exemplo, predita por alguma teoria bem estabelecida

Nós temos amplas evidências de que muitas intervenções são causais, mesmo na ausência de qualquer experimento aleatório controlado. Sabemos, por exemplo, que desfibrilação, manobra de Heimlich e uso de paraquedas são eficazes para prevenir mortes [@howick2009].

Em um artigo clássico dos primórdios da estatística escrito por @yule1899, temos um dos primeiros exemplos de utilização da regressão para estudar o efeito causal de uma política (ajuda sobre pobreza). E a despeito do título do artigo falar em causalidade ("An investigation into the causes of changes in pauperism in England, chiefly during the last two intercensal decades"), a certa altura ele diz, em nota de rodapé, que "due to" deve ser lido como "associated to". Quase cem anos depois, o grande estatístico Cox afirmaria em um artigo sobre causalidade: "it is remarkable how relatively little causality is mentioned in the general statistics literature, except in a social science context" [@cox1992, p. 292].

Um exemplo da perspectiva de que falava Cox: @muthen1987 chegou a afirmar que "It would be very healthy if more researchers abandon thinking of and using terms such as cause and effect" (p. 180). Algo ecoado em uma entrevista com um dos líderes da revolução causal desde os anos 70/80, Jamie Robins, que relata que seus papers (em journals de estatística) eram rejeitados por usar linguagem causal [@robins2022]. Então parece bastante seguro dizer que a sensação geral na estatística até os anos 80 é que não se deveria usar linguagem causal fora de estudos experimentais, isto é, em estudos observacionais.

## Da intuição comparativa ao modelo formal
...
## Resultados Potenciais (*Potential Outcomes*)

Neste capítulo, formalizamos a ideia de causalidade usando o modelo de resultados potenciais (*potential outcomes*), também conhecido como Modelo Causal de Neyman-Rubin. Esse framework nos dará uma linguagem precisa para definir efeitos causais e explicitar as suposições necessárias para estimá-los.

**Objetivos de aprendizado:**

1. Compreender a notação de resultados potenciais e a *switching equation*
2. Distinguir entre estimando, estimador e estimativa
3. Definir os principais estimandos causais: ATE, ATT e CATE
4. Enunciar as condições de identificação do ATE (ignorabilidade forte)
5. Conectar o modelo de resultados potenciais à regressão linear e interpretar o viés
...
**Suposição 1: SUTVA** (Stable Unit Treatment Value Assumption)
...
Os "?" representam os resultados potenciais que *nunca* observaremos — o contrafactual de cada unidade. Note que, por consequência, o efeito causal individual $\tau_i$ também é sempre não-observável. Essa é a essência do problema que enfrentaremos a seguir.
...
O Problema Fundamental da Inferência Causal (PFIC), batizado assim por @holland1986, diz que não podemos observar, simultaneamente, ambos os resultados potenciais $Y_i(1)$ e $Y_i(0)$.

Para tornar o PFIC concreto, considere o seguinte exemplo. Maria participa de um programa de treinamento profissional ($D_i = 1$) e, ao final, recebe um salário de R\$ 3.000 ($Y_i(1) = 3000$). Gostaríamos de saber: qual seria o salário de Maria se ela *não* tivesse participado do programa? Esse é o resultado potencial $Y_i(0)$, que nunca observaremos — Maria não pode simultaneamente ter participado e não ter participado do programa. O efeito causal individual $\tau_i = Y_i(1) - Y_i(0)$ é, portanto, fundamentalmente não-observável.

*Pare e pense:* voltando à tabela anterior, que suposição precisaríamos fazer para usar o salário de João (R\$ 2.000) como contrafactual para Maria? Essa suposição parece razoável?

Possíveis soluções para o PFIC:

1. Assumir homogeneidade temporal (comparação antes e depois da mesma unidade)
...
*Pare e pense:* em que situação o ATT e o ATE seriam exatamente iguais? E em que situações práticas esperaríamos que diferissem?
...
Em palavras, a primeira condição diz que os resultados potenciais são independentes de receber ou não o tratamento. Quando pensamos em modelos com seres humanos ou unidades com agência, a principal preocupação é que as unidades não se auto-selecionam no tratamento que lhes é mais benéfico (ou que acreditam sê-lo). Às vezes na literatura essa condição aparece como permutabilidade (exchangeability): Como dizem @hernan2020, "the treated and the untreated are exchangeable because the treated, had they remained untreated, would have experienced the same average outcome as the untreated did, and vice versa." (p. 29).

A segunda condição, conhecida como *common support* ou *overlapping condition*, diz que não existe unidade que não possa receber o tratamento ou controle. Essa condição é mais forte do que a positividade (toda unidade tem probabilidade positiva de receber o tratamento).
...
```{r common-support, echo=TRUE}
set.seed(1234)
n <- 10000
tau <- rnorm(n, 2, 2)
y_0 <- rnorm(n, 0, 2) # resultados potenciais do controle
y_1 <- tau + y_0      # resultados potenciais do tratamento
d <- rbinom(n, 1, .5) # tratamento aleatório (50/50)

# Cenário 1: common support satisfeito
y <- y_1 * d + y_0 * (1 - d)
ate <- mean(y[d == 1]) - mean(y[d == 0])
cat("ATE com common support:", round(ate, 3), "\n")

# Cenário 2: violação — unidades com Y(1) > 6 não podem ser tratadas
d1 <- ifelse(y_1 > 6, 0, d)
y_bias <- y_1 * d1 + y_0 * (1 - d1)
ate_bias <- mean(y_bias[d1 == 1]) - mean(y_bias[d1 == 0])
cat("ATE com violação de common support:", round(ate_bias, 3), "\n")
cat("Viés:", round(ate_bias - 2, 3), "\n")
```
...
Pela Lei das Expectativas Iteradas (LIE, *Law of Iterated Expectations*), temos que:
...
Mas como saber *quais* covariáveis incluir em $\mathbf{X}_i$ para que a ignorabilidade seja plausível? E quais variáveis *não* devemos condicionar? No próximo capítulo, veremos que **grafos acíclicos direcionados (DAGs)** nos oferecem uma linguagem visual e formal para responder exatamente essas perguntas.
...
**Antecipando o resultado principal:** veremos que a simples diferença de médias entre tratados e controles pode ser decomposta em três componentes: (1) o efeito causal médio $\tau_{\text{ATE}}$, que é o que gostaríamos de estimar; (2) um viés de heterogeneidade, que surge quando o efeito causal dos tratados difere da média populacional; e (3) um viés de seleção, que surge quando tratados e controles teriam resultados diferentes mesmo sem o tratamento. A álgebra a seguir nos mostrará de onde vem cada um desses termos.
...
Vamos mapeá-lo ao modelo de resultados potenciais com a *switching equation*. Primeiro, reescrevemos $Y_i$ usando a switching equation e a definição de efeito causal individual:
...
O que essa decomposição nos diz? Se $\mathbb{E}[Y_i(0)|D_i=1] \neq \mathbb{E}[Y_i(0)|D_i=0]$ — ou seja, se tratados e controles teriam resultados diferentes *mesmo sem o tratamento* — o efeito estimado é viesado. Da mesma forma, se $\mathbb{E}[\tau_i|D_i=1] \neq \tau$ — o efeito causal dos tratados difere da média populacional — também temos viés.
...
- **ATE ($\tau$):** o efeito causal médio que gostaríamos de estimar;
- **Viés de heterogeneidade ($\mathbb{E}[\tau_i|D_i=1] - \tau$):** surge quando o efeito causal dos tratados difere do efeito médio na população;
- **Viés de seleção ($\mathbb{E}[Y_i(0)|D_i=1] - \mathbb{E}[Y_i(0)|D_i=0]$):** surge quando tratados e controles teriam resultados diferentes mesmo na ausência do tratamento.
...
- **Design-based (baseado em desenho):** A identificação vem de suposições sobre o mecanismo de atribuição do tratamento. Sabemos (ou assumimos) *como* as unidades foram designadas ao tratamento ou controle, e essa informação é suficiente para identificar o efeito causal. Exemplos: experimentos aleatórios controlados (Cap. 3), regressão descontínua (Cap. 9), variáveis instrumentais quando o instrumento tem origem institucional conhecida (Cap. 7).

- **Model-based (baseado em modelo):** A identificação vem de um modelo para os resultados potenciais não-observados. Não sabemos exatamente como o tratamento foi atribuído, mas fazemos suposições sobre a estrutura dos resultados contrafactuais para estimá-los. Exemplos: diferença em diferenças com tendências paralelas (Cap. 8), controle sintético (Cap. 11), modelos de fatores interativos (Cap. 11).
...
1. **Resultados potenciais e switching equation.** Cada unidade possui dois resultados potenciais, $Y_i(1)$ e $Y_i(0)$, mas observamos apenas um deles. A switching equation $Y_i = D_i \cdot Y_i(1) + (1-D_i) \cdot Y_i(0)$ conecta os resultados potenciais ao dado observado.

2. **Estimandos causais.** Definimos os três estimandos mais comuns — ATE, ATT e CATE — e discutimos quando cada um é mais relevante para a pesquisa aplicada.

3. **Identificação.** Mostramos que, sob ignorabilidade forte (independência condicional e common support), o ATE é identificado a partir de quantidades observáveis, sem necessidade de suposições paramétricas.

4. **Conexão com regressão.** A decomposição do viés da regressão revelou que a diferença de médias simples estima o ATE apenas sob condições restritivas; caso contrário, o viés possui dois componentes — heterogeneidade do efeito causal e diferença nos resultados potenciais de base.

No próximo capítulo, introduziremos os **grafos acíclicos direcionados (DAGs)**, que oferecem uma linguagem visual e formal para representar suposições causais, complementando o framework algébrico que desenvolvemos aqui.
...
<!--chapter:end:05-SurveyExperiments.Rmd-->
...
```{r lalonde-dados, echo=TRUE, eval=TRUE, message=FALSE}
...
```{r matching-nearest, message = FALSE}
...
```{r lalonde-estimacao, message = FALSE}
...
```{r sensemakr-lalonde, message = FALSE}
library(sensemakr)
...
Table: (\#tab:dag-vs-po) Mapeamento entre as condições de IV em DAG e em resultados potenciais.
...
O lado esquerdo de \@ref(eq:itt-complier) é a forma reduzida — observável diretamente. O primeiro fator do lado direito é o LATE — o que queremos. Falta o segundo fator, $P(\text{complier})$. Sob monotonicidade, a partição em tipos satisfaz
...
A equação \@ref(eq:p-d-z1) soma always-takers e compliers — os únicos tipos que recebem $D = 1$ quando $Z = 1$. A equação \@ref(eq:p-d-z0) fica só com always-takers, já que defiers foram eliminados pela monotonicidade. Subtraindo \@ref(eq:p-d-z0) de \@ref(eq:p-d-z1):
...
Como $D_i$ é binário, $P(D_i = 1 \mid Z_i = z) = \mathbb{E}[D_i \mid Z_i = z]$ (a esperança de uma Bernoulli é a probabilidade do evento). A equação \@ref(eq:p-complier-prob) se reescreve em termos de esperanças observáveis:
...
Substituindo \@ref(eq:p-complier) em \@ref(eq:itt-complier), o segundo fator do lado direito vira a diferença observada $\mathbb{E}[D_i \mid Z_i = 1] - \mathbb{E}[D_i \mid Z_i = 0]$ no lugar de $P(\text{complier})$:
...
Para isolar o LATE, dividimos os dois lados de \@ref(eq:itt-first-stage) pela diferença $\mathbb{E}[D_i \mid Z_i = 1] - \mathbb{E}[D_i \mid Z_i = 0]$ — o primeiro estágio. Sob relevância, esse denominador é diferente de zero, e a divisão é bem definida:
...
knitr::kable(
  toy_out,
  caption = "Mini-base com tratamento cont\u00ednuo e valores de x ap\u00f3s residualiza\u00e7\u00e3o por diferentes conjuntos de efeitos fixos."
)
```
...
fig_cap_cigarro <- paste(
  "Vendas per capita de cigarros na Califórnia e nos demais estados.",
  "A linha vertical marca a aprovação da Proposição 99 em 1988."
)
fig_cap_scm_regressao <- paste(
  "Califórnia observada, contrafactual por regressão e controle sintético.",
  "A linha vertical marca a Proposição 99 em 1988."
)
fig_cap_tidysynth_trends <- paste(
  "Trajetória observada da Califórnia e controle sintético estimado pelo",
  "pacote tidysynth."
)
fig_cap_tidysynth_differences <- paste(
  "Diferença entre Califórnia observada e controle sintético ao longo do tempo."
)
fig_cap_tidysynth_weights <- paste(
  "Pesos atribuídos aos estados doadores e aos preditores no SCM estimado pelo",
  "pacote tidysynth."
)
fig_cap_tidysynth_placebos <- paste(
  "Teste placebo do SCM para a Califórnia e os estados doadores."
)
fig_cap_fect_plot <- c(
  "Efeitos dinâmicos estimados pelo FEct no exemplo da Califórnia.",
  "Efeitos dinâmicos estimados pelo IFEct no exemplo da Califórnia.",
  "Efeitos dinâmicos estimados por matrix completion no exemplo da Califórnia."
)
fig_cap_fect_placebo <- paste(
  "Teste de equivalência placebo para o modelo IFEct no exemplo da Califórnia."
)
fig_cap_did_event <- paste(
  "Event study para o estimador de DiD aplicado ao exemplo da Califórnia."
)
fig_cap_sdid_trends <- paste(
  "Comparação visual das trajetórias estimadas por DiD, controle sintético e",
  "Synthetic DiD."
)
fig_cap_sdid_weights <- paste(
  "Pesos de unidade atribuídos por DiD, controle sintético e Synthetic DiD."
)
```
...
```{r cigarro, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=fig_cap_cigarro}
library(knitr)
library(ggplot2)
library(tidyverse)
library(tidyr)
...
cigar1 <- smoking %>%
  ...
  dplyr::select(state, year, cigsale, california)

cigar1 %>%
  head() %>%
  kable(caption = "Primeiras observações dos dados de vendas de cigarros.")
...
```{r scm-regressao-prop99-outputs, include=FALSE, message=FALSE, warning=FALSE}
source("scripts/11_scm_vs_regressao_prop99.R", local = TRUE)

weight_summary_raw <- readr::read_csv(
  "outputs/scm_vs_regressao_prop99/02_resumo_pesos.csv",
  show_col_types = FALSE
)

weights_comparison_raw <- readr::read_csv(
  "outputs/scm_vs_regressao_prop99/01_pesos_scm_vs_regressao.csv",
  show_col_types = FALSE
)

counterfactual_summary_raw <- readr::read_csv(
  "outputs/scm_vs_regressao_prop99/03_contrafactual_2000.csv",
  show_col_types = FALSE
)
```

```{r tabela-pesos-regressao-scm, echo=FALSE, message=FALSE, warning=FALSE}
weight_summary_raw %>%
  mutate(
    method = dplyr::recode(
      method,
      "Regressao OLS" = "Regressão OLS"
    )
  ) %>%
  transmute(
    Método = method,
    `Soma dos pesos` = round(sum_weights, 3),
    `Peso mínimo` = round(min_weight, 3),
    `Peso máximo` = round(max_weight, 3),
    `Pesos negativos` = n_negative,
    `Pesos efetivos` = n_effectively_nonzero,
    `Norma L1` = round(l1_norm, 2)
  ) %>%
  kable(
    caption = "Resumo dos pesos usados pelo SCM e dos pesos implícitos da regressão no exemplo da Califórnia."
  )
```
...
```{r figura-contrafactual-scm-regressao, echo=FALSE, out.width="95%", fig.cap=fig_cap_scm_regressao}
knitr::include_graphics(
  "outputs/scm_vs_regressao_prop99/figura_2_contrafactual_scm_vs_regressao.png"
)
```

Em 2000, a Califórnia observada vendeu `r round(counterfactual_summary_raw$actual_california, 1)` maços per capita. A regressão prevê `r round(counterfactual_summary_raw$regression_counterfactual, 1)` e o SCM prevê `r round(counterfactual_summary_raw$scm_counterfactual, 1)`. A comparação não prova que um estimador domina o outro. Ela mostra que contrafactuais com bom ajuste pré-tratamento podem ser produzidos por objetos substantivamente diferentes.
...
**Interpretação como regressão vertical.** Também podemos ler o SCM como uma regressão ao longo da dimensão temporal. Para cada período pré-tratamento $t = 1, \ldots, T_0$, a variável dependente é $Y_{N,t}$, o resultado da unidade tratada, e as variáveis explicativas são $Y_{1,t}, \ldots, Y_{N_0,t}$, os resultados dos doadores. O SCM busca pesos $\omega_i$ que resolvam:

$$
Y_{N,t} \approx \sum_{i=1}^{N_0} \omega_i Y_{i,t}, \quad \text{para } t = 1, \ldots, T_0
$$

Quando $N_0 > T_0$, há mais doadores do que períodos pré-tratamento. O sistema pode ficar subdeterminado: diferentes combinações ajustam bem o pré-tratamento, mas divergem no pós-tratamento. As restrições de convexidade funcionam como regularização. Elas restringem o espaço de soluções, costumam produzir poucos pesos substantivos e tornam o contrafactual inspecionável.

A tabela a seguir ilustra a estrutura do problema. A última coluna é a unidade tratada; as demais são doadores. O SCM ajusta as linhas pré-tratamento da unidade tratada usando as demais colunas:

```{r estrutura-scm-classico, echo=FALSE}
data.frame(
  Período = c("$t=1$", "$\\vdots$", "$t=T_0$", "$t=T_0+1$", "$\\vdots$", "$t=T$"),
  `Doador 1` = c("$Y_{1,1}$", "$\\vdots$", "$Y_{1,T_0}$", "$Y_{1,T_0+1}$", "$\\vdots$", "$Y_{1,T}$"),
  `Doador 2` = c("$Y_{2,1}$", "$\\vdots$", "$Y_{2,T_0}$", "$Y_{2,T_0+1}$", "$\\vdots$", "$Y_{2,T}$"),
  `$\\cdots$` = c("$\\cdots$", "", "$\\cdots$", "$\\cdots$", "", "$\\cdots$"),
  `Doador $N_0$` = c("$Y_{N_0,1}$", "$\\vdots$", "$Y_{N_0,T_0}$", "$Y_{N_0,T_0+1}$", "$\\vdots$", "$Y_{N_0,T}$"),
  Tratada = c("$Y_{N,1}$", "$\\vdots$", "$Y_{N,T_0}$", "$Y_{N,T_0+1}(1)$", "$\\vdots$", "$Y_{N,T}(1)$"),
  check.names = FALSE
) %>%
  kable(
    caption = "Estrutura de dados do SCM clássico, com períodos nas linhas e unidades nas colunas.",
    escape = FALSE
  )
```

As linhas pré-tratamento são usadas para estimar os pesos. As linhas pós-tratamento projetam o contrafactual $\hat{Y}_{N,t}(0) = \sum_i \hat{\omega}_i Y_{i,t}$.
...
Abadie e L'Hour (2021) propõem uma solução elegante: adicionar uma penalidade que favorece pesos associados a doadores **mais similares** à unidade tratada. O problema modificado é:
...
sujeito a $\omega_i \geq 0$ e $\sum_i \omega_i = 1$, com $\lambda > 0$. O segundo termo penaliza o uso de doadores cujas trajetórias individuais são distantes da unidade tratada. Isso restaura a unicidade da solução e reduz o viés de interpolação. Robbins et al. (2017) propõem uma abordagem alternativa: escolher, dentre as soluções ótimas, os pesos mais próximos de pesos iguais — evitando dependência excessiva de poucos doadores.
...
```{r cigarro-tidysynth, echo=TRUE, message=FALSE, warning=FALSE, cache=FALSE, fig.cap=fig_cap_tidysynth_trends}
library(tidysynth)
library(tidyverse)
data(smoking)
...
```{r cigarro-tidysynth-diferencas, echo=TRUE, message=FALSE, warning=FALSE, fig.cap=fig_cap_tidysynth_differences}
smoking_out %>% plot_differences()
```

Os pesos atribuídos a cada estado e a cada variável preditora:

```{r cigarro-tidysynth-pesos, echo=TRUE, message=FALSE, warning=FALSE, fig.cap=fig_cap_tidysynth_weights}
smoking_out %>% plot_weights()
```

O balanceamento entre a Califórnia real e a sintética nos preditores:

```{r cigarro-tidysynth-balanceamento, echo=TRUE, message=FALSE, warning=FALSE}
smoking_out %>%
  grab_balance_table() %>%
...
```{r cigarro-tidysynth-placebos, echo=TRUE, message=FALSE, warning=FALSE, fig.cap=fig_cap_tidysynth_placebos}
...
$$
\text{p-valor} = \frac{1}{N} \sum_{i=1}^{N} \mathbf{1}[R_i \geq R_N]
$$

onde $R_i = \sum_{t > T_0} (Y_{it} - \hat{Y}_{it}(0))^2$ é o erro de predição pós-tratamento da unidade $i$.
...
**Complicações adicionais na prática.** Mesmo aceitando a lógica do teste, duas complicações surgem. Primeira: para algumas unidades placebo, o controle sintético pode ter ajuste pré-tratamento ruim — nesse caso, seu $R_i$ será alto por falha de ajuste, não por efeito causal. Uma solução é descartar placebos com RMSPE pré-tratamento acima de um limiar, ou usar a *razão* entre RMSPE pós e pré (em vez do nível) como estatística de teste. Segunda: com poucas unidades ($N$ pequeno), o menor p-valor possível é $1/N$ — com $N = 39$ estados (como no exemplo da Califórnia), o menor p-valor é $\approx 0.026$, o que limita a precisão inferencial.
...
```{r fect-dinamico, echo=TRUE, message=FALSE, warning=FALSE, fig.width=10, fig.height=4, cache=TRUE, fig.cap=fig_cap_fect_plot}
par(mfrow = c(1, 3))
...
```{r fect-placebo, echo=TRUE, message=FALSE, warning=FALSE, fig.width=8, fig.height=5, cache=TRUE, fig.cap=fig_cap_fect_placebo}
...
```{r did1, echo=TRUE, message=FALSE, warning=FALSE, fig.cap=fig_cap_did_event}
library(did2s)
library(ggfixest)
...
```{r sdid1, echo=TRUE, message=FALSE, warning=FALSE, cache=TRUE, fig.cap=fig_cap_sdid_trends}
...
```{r sdid2, echo=TRUE, message=FALSE, warning=FALSE, cache=TRUE, fig.cap=fig_cap_sdid_weights}
...
```{r tabela-guia-metodos-scm, echo=FALSE}
tibble::tribble(
  ...
  kable(
    caption = "Resumo das suposições, pesos e pacotes dos principais estimadores contrafactuais discutidos no capítulo.",
    escape = FALSE
  )
```
...
$$
Em que $X$ é a variável causal de interesse (tratamento) e os $W$ são variáveis de controle. São as *nuisance variables*. O teorema de FWL diz que posso estimar $\beta_1$ rodando diretamente a regressão acima, ou então pelos seguintes passos:
...
3. Rodo uma regressão de $Y$ nos controles $W$ e obtenho os resíduos $\tilde{Y} = Y - \hat{Y}$. Finalmente, rodo a regressão $\tilde{Y} = \beta_1 \tilde{X} + \epsilon$ para obter $\hat{\beta}_1$.
...
Primeiro, construímos o instrumento Bartik: a mudança *esperada* na votação de cada estado, com base na composição partidária inicial (2014) e nas tendências nacionais de cada bloco.
...
### Voltando ao exemplo: construindo $w_{ik}$, $g_k$ e $B_i$ com dados eleitorais
...
As **shares** $w_{ik}$ são a composição partidária de cada estado em 2014 (o período-base $t_0$):
...
O PT caiu cerca de 16pp nacionalmente, o PSDB desmoronou ~26pp, e o bloco Outros (liderado por Bolsonaro e Ciro) subiu ~43pp. Esses choques são os mesmos para todos os estados --- são tendências **nacionais**.

Finalmente, o **instrumento Bartik** $B_i = \sum_k w_{ik} \cdot g_k$ combina as shares iniciais com os shifts nacionais:

```{r ss-iv-bartik}
tab_bartik <- data.frame(
  UF = eleicoes$UF,
  Bartik = round(bartik * 100, 1)
)
tab_bartik_sel <- tab_bartik[tab_bartik$UF %in% c("BA","CE","MG","SP","RS"), ]
tab_bartik_sel <- tab_bartik_sel[match(c("BA","CE","MG","SP","RS"), tab_bartik_sel$UF), ]
kable(tab_bartik_sel, row.names = FALSE,
      caption = "Instrumento Bartik $B_i$ para cada estado (pp)",
      col.names = c("Estado ($i$)", "$B_i$ (pp)"))
```

Observe que **os shifts $g_k$ são idênticos para todos os estados** --- o que diferencia os $B_i$ entre si são exclusivamente as shares $w_{ik}$. A Bahia, com 66% de votos no PT em 2014, é muito mais exposta à queda nacional do PT do que São Paulo, com 31%. É essa variação *cross-sectional* no instrumento, gerada pela composição inicial, que permite a identificação.
...
### Interpretando o modelo com o exemplo eleitoral

Vamos dar concretude ao modelo acima usando nosso exemplo. Suponha que queremos estimar o efeito causal da **mudança na votação do PT** ($X_i$ = queda do PT em pp no estado $i$) sobre alguma variável de resultado político $Y_i$ --- por exemplo, o nível de abstenção, o gasto público municipal ou a ocorrência de protestos.
...
**Primeiro estágio**: O instrumento Bartik $B_i$ prevê a queda do PT no estado $i$. A lógica é que estados com maior concentração inicial de votos no PT ($w_{i,PT}$ alto) estão mecanicamente mais expostos à tendência nacional de queda do PT. O primeiro estágio estima:
...
**Segundo estágio**: Substituímos $X_i$ por seu valor predito $\hat{X}_i$ --- a parte da mudança local que é explicada *apenas* pelas tendências nacionais interagidas com a composição inicial:

$$Y_i = \alpha + \beta \hat{X}_i + \eta_i$$

A estimativa $\hat{\beta}$ captura o efeito causal de $X_i$ sobre $Y_i$, **desde que o instrumento satisfaça a restrição de exclusão**. Intuitivamente, ao usar $\hat{X}_i$ em vez de $X_i$, estamos "limpando" a variação endógena (fatores locais) e ficando apenas com a variação exógena (exposição a tendências nacionais via composição inicial).
...
```{r dag-ss, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="DAG do instrumento shift-share. O instrumento $B_i$ é construído a partir das shares iniciais ($w$) e dos choques nacionais ($g$). A restrição de exclusão exige que não haja caminho direto de $B$ para $Y$ que não passe por $X$."}
library(ggdag)
...
Xu (2025, *Journal of Politics*) --- "Bureaucratic Packing in the Brazilian Amazon" --- é uma aplicação de SSIV em ciência política brasileira que exemplifica como resolver os problemas de identificação discutidos na seção sobre exclusion restriction. A pergunta central é: **competição política local causa desmatamento na Amazônia?**
...
**A estratégia de identificação.** Xu adapta o método de Shaukat (2019) para construir um instrumento shift-share que prevê competição política municipal a partir de tendências eleitorais estaduais. A construção tem três componentes:

- **Shares** ($z_{p,m,s}$): a votação do partido $p$ no município $m$ do estado $s$ na eleição estadual de 1998, o período-base. Essas *shares* capturam a composição partidária inicial do município.

- **Shifts** ($g_{p,s,t}^{-m}$): a mudança na votação do partido $p$ no nível **estadual** entre 1998 e a eleição $t$, **excluindo** o próprio município $m$ do cálculo (leave-out). Esses *shifts* capturam tendências partidárias estaduais que são plausivamente exógenas ao município individual.

- **Predicted vote share**: $\hat{s}_{p,m,s,t} = z_{p,m,s} + g_{p,s,t}^{-m}$. O instrumento combina a composição inicial do município com a tendência estadual para prever a votação de cada partido no município.

A partir dos vote shares preditos, Xu constrói uma medida de **competição política predita**: $1 - |\hat{s}_{winner} - \hat{s}_{runner-up}|$, onde valores mais altos indicam margens menores entre os dois candidatos mais votados (mais competição).

**Por que essa estratégia funciona?** Três elementos-chave garantem a identificação:

1. **Variação cross-level**: Os *shifts* vêm de eleições **estaduais**, mas os outcomes (desmatamento) são **municipais**. Ao usar tendências de um nível geográfico mais agregado para prever outcomes em um nível mais desagregado, a separação entre o instrumento e confundidores locais não observados se torna mais plausível. Isso contrasta com o nosso exemplo hipotético anterior, onde shifts e outcomes operavam no mesmo nível estadual.

2. **Leave-out design**: Ao excluir o próprio município $m$ do cálculo do shift estadual $g_{p,s,t}^{-m}$, elimina-se a correlação mecânica entre o instrumento e choques locais não observados. Se um choque local afeta tanto o desmatamento quanto a votação no município $m$, esse choque não contamina o shift estadual porque $m$ foi excluído.

3. **Nonmonotonicity**: Um partido ganhando popularidade estadual *aumenta* a competição em municípios onde era fraco, mas *diminui* a competição onde já era forte. Essa propriedade gera variação rica no instrumento e torna improvável que tendências estaduais afetem o desmatamento municipal por um canal direto uniforme.

**Resultado.** Xu encontra que maior competição política leva a **mais desmatamento**. O mecanismo identificado é o *bureaucratic packing*: prefeitos em eleições competitivas nomeiam mais funcionários comissionados para construir máquinas eleitorais, deslocando recursos da fiscalização ambiental. O enfraquecimento da capacidade de enforcement local facilita o desmatamento ilegal.

**Conexão com as estratégias de identificação.** O design de Xu/Shaukat aposta fundamentalmente na exogeneidade dos *shifts* --- as tendências estaduais de votação partidária, excluindo o município. Isso o alinha à estratégia BHJ (2022): a identificação vem da quase-aleatoriedade dos choques, não da exogeneidade das shares iniciais. A combinação de variação cross-level com leave-out design oferece um modelo replicável para aplicações de SSIV em ciência política comparada, especialmente em contextos onde a unidade de tratamento (município) está aninhada em unidades políticas maiores (estado).
...
Xu, Alice Z. 2025. "Bureaucratic Packing in the Brazilian Amazon: How Political Competition Drives Deforestation." *Journal of Politics* 87(4): 1350-1363.

<!--chapter:end:15-Shift-Share.Rmd-->
