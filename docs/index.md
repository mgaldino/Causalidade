...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/fig2_classificacao} \caption{Classificação sequencial dos tipos de estudos em ciência política. Dos 91.632 artigos, o fluxo vai de todos os estudos para quantitativos, depois para descritivos, preditivos e explanatórios, e finalmente para design-based vs. model-based. Fonte: Torreblanca et al. (2026).}(\#fig:fig-classificacao)
\end{figure}
...
\begin{figure}
\includegraphics[width=0.9\linewidth]{figures/fig5_metodos} \caption{Proporção de cada método design-based entre os estudos explanatórios quantitativos, por ano (2003–2023). Fonte: Torreblanca et al. (2026).}(\#fig:fig-metodos)
\end{figure}
...
read_data <- function(df) {
  local_path <- file.path("data", df)
  if (!file.exists(local_path)) {
    stop("Arquivo local não encontrado: ", local_path)
  }
