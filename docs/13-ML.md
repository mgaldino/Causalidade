...
![](13-ML_files/figure-latex/cap13-sim2-1.pdf)<!-- --> 

``` r
hist(my_p_values_beta$beta, breaks = 30, main = "Teste de signific. de D", xlab = "p-valor")
```

![](13-ML_files/figure-latex/cap13-sim2-2.pdf)<!-- --> 
...
``` r
# Vetor para armazenar se D foi selecionado pelo LASSO
...
for (i in 1:1000) {
  y <- lista_df[[i]]$y
  control_cols <- grep("^X[0-9]+$", names(lista_df[[i]]), value = TRUE)
  X <- lista_df[[i]] |>
    dplyr::select(D, dplyr::all_of(control_cols)) |>
    as.matrix()
  ...
  lasso_model <- cv.glmnet(X, y, alpha = 1, foldid = foldid) # alpha = 1 para LASSO

  # Extraindo os coeficientes no valor de lambda que minimiza o erro
  lasso_coefs <- coef(lasso_model, s = "lambda.min")

  # Verificando se D foi selecionado pelo LASSO (coeficiente diferente de zero)
  lasso_selected_D[i] <- ifelse(lasso_coefs["D", 1] != 0, 1, 0)
}

# Analisando os resultados
hist(lasso_selected_D, breaks = 40, main = "Seleção de D pelo LASSO", xlab = "D selecionado (1 = sim, 0 = não)")
```

![](13-ML_files/figure-latex/cap13-sim3-1.pdf)<!-- --> 
...
![](13-ML_files/figure-latex/cap-13-hdm-1.pdf)<!-- --> 
